...
parser.add_argument('--training_samples_number', type = str, default = '300mats',
...
parser.add_argument('--grad_calculate', type = eval, default = False)
...
def main(training_time = 3, epoch_num = 100, batch_size = 64, lr = 1e-3, minlr = 1e-9):
    ...
    for tt in range(training_time):
        ...
        speed_Bs = torch.tensor([1], device = device, requires_grad = False) # save v(n)
        speed_bt = torch.tensor([1], device = device, requires_grad = False)
        speed_Up = torch.tensor([1], device = device, requires_grad = False)
        
        old_loss_Bs = torch.tensor([1.], device = device, requires_grad = False) # save L(n - 1)
        old_loss_bt = torch.tensor([1.], device = device, requires_grad = False)
        old_loss_Up = torch.tensor([1.], device = device, requires_grad = False)
        ...
        for e in range(epoch_num):
            ...
            # update weight
            speed_Bs = args.mu * old_loss_Bs / losses_Bs + (1 - args.mu) * speed_Bs
            old_loss_Bs = losses_Bs
            
            speed_bt = args.mu * old_loss_bt / losses_bt + (1 - args.mu) * speed_bt
            old_loss_bt = losses_bt
            
            speed_Up = args.mu * old_loss_Up / losses_Up + (1 - args.mu) * speed_Up
            old_loss_Up = losses_Up
            ...
            if e == epoch_num - 1:
                model_name = args.multi_task_model_name + '_' + args.mmWave_network_scenarios + '_' + args.training_samples_number + '_velocity' + args.UE_velocity + '_snr' + args.SNR + '_' + args.move_form + '_tt' + str(tt) + '.pkl'
