...
from model_MTL import Bs, bt, Up, Vanilla, Bs2bt2Up, Up2bt2Bs, Dual_Cascaded
import sys
import time
import argparse
import logging
...
# scenario parameters
parser.add_argument('--experiment_type', type = str, default = 'Outdoor_Blockage_trainingsamples',
                    choices = ['O1_trainingsamples', 'O1_velocity', 'O1_snr', 'O1_motion_form', 'Outdoor_Blockage_trainingsamples'])
parser.add_argument('--visualize', type = eval, default = True)
parser.add_argument('--save_path', type = str, default = 'results')
parser.add_argument('--save_matlab', type = eval, default = True)
...
def main(training_time = 3, epoch_num = 100, batch_size = 64):
    ...
    if args.experiment_type == 'O1_trainingsamples':
        ...
        model_Bs = Bs(his_len = his_len, pre_len = pre_len, BS_num = BS_num, beam_num = beam_num, BS_dim = BS_dim, device = device)
        model_bt = bt(his_len = his_len, pre_len = pre_len, BS_num = BS_num, beam_num = beam_num, BS_dim = BS_dim, device = device)
        model_Up = Up(his_len = his_len, pre_len = pre_len, BS_num = BS_num, beam_num = beam_num, BS_dim = BS_dim, device = device)        
        model_Vanilla = Vanilla(his_len = his_len, 
                      pre_len = pre_len, 
                      BS_num = BS_num, 
                      beam_num = beam_num, 
                      cnn_feature_num = 64, 
                      ...
                      BS_dim = BS_dim, 
                      device = device)
        model_Bs2bt2Up = Bs2bt2Up(his_len = his_len, 
                      pre_len = pre_len, 
                      BS_num = BS_num, 
                      beam_num = beam_num, 
                      cnn_feature_num = 64, 
                      ...
                      BS_dim = BS_dim, 
                      device = device)
        model_Up2bt2Bs = Up2bt2Bs(his_len = his_len, 
                      pre_len = pre_len, 
                      BS_num = BS_num, 
                      beam_num = beam_num, 
                      cnn_feature_num = 64, 
                      ...
                      BS_dim = BS_dim, 
                      device = device)
        model_Dual_Cascaded = Dual_Cascaded(his_len = his_len, 
                      pre_len = pre_len, 
                      BS_num = BS_num, 
                      beam_num = beam_num, 
                      cnn_feature_num = 64, 
        ...
        for mat_count in range(len(mat_list)):
            ...
            model_name = 'trained_model/Dual_Cascaded_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'  
            model_Dual_Cascaded.load_state_dict(torch.load(model_name))       
            model_Dual_Cascaded.to(device)
            model_Dual_Cascaded.device = device
            model_Dual_Cascaded.eval()
                        
            BS_acur, beam_acur, beam_norm_gain, dis = eval_Bs_bt_Up(model_Dual_Cascaded, eval_loader, b, his_len, pre_len, BS_num, beam_num, device)
            ...
            model_name = 'trained_model/Bs2bt2Up_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Bs2bt2Up.load_state_dict(torch.load(model_name))          
            model_Bs2bt2Up.to(device)
            model_Bs2bt2Up.device = device
            model_Bs2bt2Up.eval()
                        
            BS_acur, beam_acur, beam_norm_gain, dis = eval_Bs_bt_Up(model_Bs2bt2Up, eval_loader, b, his_len, pre_len, BS_num, beam_num, device)
            ...
            model_name = 'trained_model/Up2bt2Bs_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Up2bt2Bs.load_state_dict(torch.load(model_name))                   
            model_Up2bt2Bs.to(device)
            model_Up2bt2Bs.device = device
            model_Up2bt2Bs.eval()
                        
            BS_acur, beam_acur, beam_norm_gain, dis = eval_Bs_bt_Up(model_Up2bt2Bs, eval_loader, b, his_len, pre_len, BS_num, beam_num, device)
            ...
            model_name = 'trained_model/Vanilla_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Vanilla.load_state_dict(torch.load(model_name))            
            model_Vanilla.to(device)
            model_Vanilla.device = device
            model_Vanilla.eval()
                        
            BS_acur, beam_acur, beam_norm_gain, dis = eval_Bs_bt_Up(model_Vanilla, eval_loader, b, his_len, pre_len, BS_num, beam_num, device)
            ...
            model_name = 'trained_model/STL_BS_selection_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Bs.load_state_dict(torch.load(model_name))                    
            model_Bs.to(device)
            model_Bs.device = device
            model_Bs.eval()
                        
            BS_acur = eval_Bs(model_Bs, eval_loader, b, his_len, pre_len, BS_num, beam_num, device)
            
            model_name = 'trained_model/STL_beam_tracking_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_bt.load_state_dict(torch.load(model_name))              
            model_bt.to(device)
            model_bt.device = device
            model_bt.eval()
            
            beam_acur, beam_norm_gain = eval_bt(model_bt, eval_loader, b, his_len, pre_len, BS_num, beam_num, device)
            
            model_name = 'trained_model/STL_UE_positioning_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Up.load_state_dict(torch.load(model_name))         
            model_Up.to(device)
            model_Up.device = device
            model_Up.eval()
            
            dis = eval_Up(model_Up, eval_loader, b, his_len, pre_len, BS_num, beam_num, device)
        ...
        if args.save_matlab:
            ...
            mat_name = args.experiment_type + '.mat'
            sio.savemat(mat_name, {'BS_acur_eval': BS_acur_eval,
        ...
        for velocity_count in range(len(velocity_list)):
            ...
            print('load dual-cascaded multi-task model!')         
            model_name = 'trained_model/Dual_Cascaded_O1_300mats_velocity' + velocity_list[velocity_count] + '_snr5dB_rectilinear_motion.pth'           
            model_Dual_Cascaded.load_state_dict(torch.load(model_name))       
            model_Dual_Cascaded.to(device)
            model_Dual_Cascaded.device = device
            model_Dual_Cascaded.eval()
            ...
            print('load single-cascaded multi-task model!')         
            model_name = 'trained_model/Bs2bt2Up_O1_300mats_velocity' + velocity_list[velocity_count] + '_snr5dB_rectilinear_motion.pth'
            model_Bs2bt2Up.load_state_dict(torch.load(model_name))          
            model_Bs2bt2Up.to(device)
            model_Bs2bt2Up.device = device
            model_Bs2bt2Up.eval()
            ...
            print('load inverse single-cascaded multi-task model!')         
            model_name = 'trained_model/Up2bt2Bs_O1_300mats_velocity' + velocity_list[velocity_count] + '_snr5dB_rectilinear_motion.pth'      
            model_Up2bt2Bs.load_state_dict(torch.load(model_name))                   
            model_Up2bt2Bs.to(device)
            model_Up2bt2Bs.device = device
            model_Up2bt2Bs.eval()
            ...
            print('load vanilla multi-task model!')         
            model_name = 'trained_model/Vanilla_O1_300mats_velocity' + velocity_list[velocity_count] + '_snr5dB_rectilinear_motion.pth'               
            model_Vanilla.load_state_dict(torch.load(model_name))            
            model_Vanilla.to(device)
            model_Vanilla.device = device
            model_Vanilla.eval()
            ...
            print('load single-task learning model!')         
            model_name = 'trained_model/STL_BS_selection_O1_300mats_velocity' + velocity_list[velocity_count] + '_snr5dB_rectilinear_motion.pth'               
            model_Bs.load_state_dict(torch.load(model_name))                    
            model_Bs.to(device)
            model_Bs.device = device
            model_Bs.eval()
            ...
            model_name = 'trained_model/STL_beam_tracking_O1_300mats_velocity' + velocity_list[velocity_count] + '_snr5dB_rectilinear_motion.pth'    
            model_bt.load_state_dict(torch.load(model_name))              
            model_bt.to(device)
            model_bt.device = device
            model_bt.eval()
            ...
            model_name = 'trained_model/STL_UE_positioning_O1_300mats_velocity' + velocity_list[velocity_count] + '_snr5dB_rectilinear_motion.pth'     
            model_Up.load_state_dict(torch.load(model_name))         
            model_Up.to(device)
            model_Up.device = device
            model_Up.eval()
        ...
        for snr_count in range(len(snr_list)):
            ...
            print('load dual-cascaded multi-task model!')         
            model_name = 'trained_model/Dual_Cascaded_O1_300mats_velocity10ms_snr' + snr_list[snr_count] + '_rectilinear_motion.pth'           
            model_Dual_Cascaded.load_state_dict(torch.load(model_name))       
            model_Dual_Cascaded.to(device)
            model_Dual_Cascaded.device = device
            model_Dual_Cascaded.eval()
            ...
            print('load single-cascaded multi-task model!')         
            model_name = 'trained_model/Bs2bt2Up_O1_300mats_velocity10ms_snr' + snr_list[snr_count] + '_rectilinear_motion.pth'
            model_Bs2bt2Up.load_state_dict(torch.load(model_name))          
            model_Bs2bt2Up.to(device)
            model_Bs2bt2Up.device = device
            model_Bs2bt2Up.eval()
            ...
            print('load inverse single-cascaded multi-task model!')          
            model_name = 'trained_model/Up2bt2Bs_O1_300mats_velocity10ms_snr' + snr_list[snr_count] + '_rectilinear_motion.pth'
            model_Up2bt2Bs.load_state_dict(torch.load(model_name))                   
            model_Up2bt2Bs.to(device)
            model_Up2bt2Bs.device = device
            model_Up2bt2Bs.eval()
            ...
            print('load vanilla multi-task model!')         
            model_name = 'trained_model/Vanilla_O1_300mats_velocity10ms_snr' + snr_list[snr_count] + '_rectilinear_motion.pth'            
            model_Vanilla.load_state_dict(torch.load(model_name))            
            model_Vanilla.to(device)
            model_Vanilla.device = device
            model_Vanilla.eval()
            ...
            print('load single-task learning model!')         
            model_name = 'trained_model/STL_BS_selection_O1_300mats_velocity10ms_snr' + snr_list[snr_count] + '_rectilinear_motion.pth'                         
            model_Bs.load_state_dict(torch.load(model_name))                    
            model_Bs.to(device)
            model_Bs.device = device
            model_Bs.eval()
            ...
            model_name = 'trained_model/STL_beam_tracking_O1_300mats_velocity10ms_snr' + snr_list[snr_count] + '_rectilinear_motion.pth'  
            model_bt.load_state_dict(torch.load(model_name))              
            model_bt.to(device)
            model_bt.device = device
            model_bt.eval()
            ...
            model_name = 'trained_model/STL_UE_positioning_O1_300mats_velocity10ms_snr' + snr_list[snr_count] + '_rectilinear_motion.pth'  
            model_Up.load_state_dict(torch.load(model_name))         
            model_Up.to(device)
            model_Up.device = device
            model_Up.eval()
        ...
        for mat_count in range(len(mat_list)):
            ...
            print('load dual-cascaded multi-task model!')         
            model_name = 'trained_model/Dual_Cascaded_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_spiral_motion.pth'           
            model_Dual_Cascaded.load_state_dict(torch.load(model_name))       
            model_Dual_Cascaded.to(device)
            model_Dual_Cascaded.device = device
            model_Dual_Cascaded.eval()
            ...
            print('load single-cascaded multi-task model!')         
            model_name = 'trained_model/Bs2bt2Up_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_spiral_motion.pth'           
            model_Bs2bt2Up.load_state_dict(torch.load(model_name))          
            model_Bs2bt2Up.to(device)
            model_Bs2bt2Up.device = device
            model_Bs2bt2Up.eval()
            ...
            print('load inverse single-cascaded multi-task model!')         
            model_name = 'trained_model/Up2bt2Bs_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_spiral_motion.pth'           
            model_Up2bt2Bs.load_state_dict(torch.load(model_name))                   
            model_Up2bt2Bs.to(device)
            model_Up2bt2Bs.device = device
            model_Up2bt2Bs.eval()
            ...
            print('load vanilla multi-task model!')         
            model_name = 'trained_model/Vanilla_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_spiral_motion.pth'           
            model_Vanilla.load_state_dict(torch.load(model_name))            
            model_Vanilla.to(device)
            model_Vanilla.device = device
            model_Vanilla.eval()
            ...
            print('load single-task learning model!')         
            model_name = 'trained_model/STL_BS_selection_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_spiral_motion.pth'           
            model_Bs.load_state_dict(torch.load(model_name))                    
            model_Bs.to(device)
            model_Bs.device = device
            model_Bs.eval()
            ...
            model_name = 'trained_model/STL_beam_tracking_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_spiral_motion.pth'           
            model_bt.load_state_dict(torch.load(model_name))              
            model_bt.to(device)
            model_bt.device = device
            model_bt.eval()
            ...
            model_name = 'trained_model/STL_UE_positioning_O1_' + mat_list[mat_count] + '_velocity10ms_snr5dB_spiral_motion.pth'           
            model_Up.load_state_dict(torch.load(model_name))         
            model_Up.to(device)
            model_Up.device = device
            model_Up.eval()
        ...
        for mat_count in range(len(mat_list)):
            ...
            print('load dual-cascaded multi-task model!')         
            model_name = 'trained_model/Dual_Cascaded_O1_Blockage_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Dual_Cascaded.load_state_dict(torch.load(model_name))       
            model_Dual_Cascaded.to(device)
            model_Dual_Cascaded.device = device
            model_Dual_Cascaded.eval()
            ...
            print('load single-cascaded multi-task model!')         
            model_name = 'trained_model/Bs2bt2Up_O1_Blockage_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Bs2bt2Up.load_state_dict(torch.load(model_name))          
            model_Bs2bt2Up.to(device)
            model_Bs2bt2Up.device = device
            model_Bs2bt2Up.eval()
            ...
            print('load inverse single-cascaded multi-task model!')         
            model_name = 'trained_model/Up2bt2Bs_O1_Blockage_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Up2bt2Bs.load_state_dict(torch.load(model_name))                   
            model_Up2bt2Bs.to(device)
            model_Up2bt2Bs.device = device
            model_Up2bt2Bs.eval()
            ...
            print('load vanilla multi-task model!')         
            model_name = 'trained_model/Vanilla_O1_Blockage_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Vanilla.load_state_dict(torch.load(model_name))            
            model_Vanilla.to(device)
            model_Vanilla.device = device
            model_Vanilla.eval()
            ...
            print('load single-task learning model!')         
            model_name = 'trained_model/STL_BS_selection_O1_Blockage_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Bs.load_state_dict(torch.load(model_name))                    
            model_Bs.to(device)
            model_Bs.device = device
            model_Bs.eval()
            ...
            model_name = 'trained_model/STL_beam_tracking_O1_Blockage_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_bt.load_state_dict(torch.load(model_name))              
            model_bt.to(device)
            model_bt.device = device
            model_bt.eval()
            ...
            model_name = 'trained_model/STL_UE_positioning_O1_Blockage_' + mat_list[mat_count] + '_velocity10ms_snr5dB_rectilinear_motion.pth'           
            model_Up.load_state_dict(torch.load(model_name))         
            model_Up.to(device)
            model_Up.device = device
            model_Up.eval()
