...
for i = row_index
    ...
    % MM_file = ['./DeepMIMO_BS1-4-8-9_Row250-2500_dataset/dataset_row' num2str(read_idx) '.mat'];
    MM_file = ['./DeepMIMO_BS2-3-6_Row500-1500_dataset/dataset_row' num2str(read_idx) '.mat'];
    load(MM_file);
    ...
    for i = 1 : file_num
        for j = 1 : batch_num
            ...
            while flag == 0
                ...
                initial_y = round(900 + rand * 200); % U(900, 1100) for Row875-1125
                direction = rand * 2 * pi; % U(0, 2 * pi)
                a = rand * speed * 0.2; % accelaration, U(0, 0.2v)
                % beam tracking duration: 1.6 s
                % beam training and prediction period: 0.16 s
                location = round([initial_x, initial_y] + (speed / 0.2 * [0 : 0.16 : 1.6 - 0.16].' + ...
                    0.5 * a / 0.2 * ([0 : 0.16 : 1.6 - 0.16].^2).') * [cos(direction), sin(direction)]); % divide 0.2, since the spacing of adjacent sample is 20cm
        ...
        save_filename = ['./O1_28B_BS2-3-6_Row875-1125_MTLdataset/v' num2str(speed) 'ms_snr' num2str(SNR) 'dB/train_500mats/dataset_' num2str(i) '.mat'];
